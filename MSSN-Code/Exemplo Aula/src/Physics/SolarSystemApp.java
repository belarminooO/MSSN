...
float[] viewport = {0.0f, 0.0f, 1.0f, 1.0f};  // Ocupa toda a tela
    ...
    public void mousePressed(PApplet p) {
        ...
        public void mousePressed(PApplet p) {
            ...
            // Gera velocidade, massa, raio e cor aleatórios
            PVector vel = new PVector(p.random() * 2e4f - 1e4f, random.nextFloat() * 2e4f - 1e4f); // Velocidade aleatória
            float mass = random.nextFloat() * 5e24f + 1e23f; // Massa aleatória
            float radius = distEarthSun / 30 * (random.nextFloat() * 0.5f + 0.5f); // Raio aleatório
            int color = p.color(random.nextInt(256), random.nextInt(256), random.nextInt(256)); // Cor aleatória

            // Cria o novo corpo e o adiciona à lista
